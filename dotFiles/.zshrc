...
fif() {
  out=$(rg \
	...
	| fzf --preview 'bat --wrap character --color always {} --highlight-line {} --line-range {}' --preview-window wrap)
        
  if [[ -z "$out" ]]
  then return
  fi
  read -r filename line <<< "${out}"
  vi "${filename}" +"normal! ${line}zz"
}
...
# Aliases
alias vi="nvim"
alias notes='selected=$(find ~/Obsidian/RTH | grep .md | fif)'
